...
export function QRGenerator() {
    ...
    useEffect(() => {
        ...
        let isEmpty = true;
        switch (qrType) {
            case 'url': case 'text': finalDataString = inputValue; isEmpty = !inputValue.trim(); break;
            case 'email': finalDataString = `mailto:${emailData.to}?subject=${encodeURIComponent(emailData.subject)}`; isEmpty = !emailData.to.trim(); break;
            case 'sms': finalDataString = `SMSTO:${smsData.number}:${encodeURIComponent(smsData.message)}`; isEmpty = !smsData.number.trim(); break;
            case 'wifi': finalDataString = `WIFI:T:${wifiData.encryption};S:${wifiData.ssid};P:${wifiData.password};;`; isEmpty = !wifiData.ssid.trim(); break;
            case 'phone': finalDataString = `tel:${phoneData.country.dial_code}${phoneData.number}`; isEmpty = !phoneData.number.trim(); break;
        }
        setFinalQrData(finalDataString);
        if (isEmpty) {
            setQrCodeDataUrl('');
        } else {
            generateQR(finalDataString);
        }
    }, [inputValue, emailData, smsData, wifiData, phoneData, qrType, colorDark, colorLight, logo, frame]);
    ...
    const handleSvgDownload = () => {
        ...
        const options = { errorCorrectionLevel: 'H' as QRErrorCorrectionLevel, color: { dark: colorDark, light: colorLight }, };
        QRCode.toString(finalQrData, options, (err, svgString) => {
    ...
    const handleTypeChange = (newType: QRType) => {
        ...
        setPhoneData({ country: countryCodes[0], number: '' });
    };
    ...
    return (
        <div className="container max-w-5xl mx-auto bg-white rounded-2xl shadow-2xl p-6 md:p-10 grid lg:grid-cols-2 gap-10">
            <div className="flex flex-col space-y-6">
                ...
                <div className="bg-light-bg p-4 rounded-lg border"> <h2 className="font-semibold text-lg mb-4 flex items-center gap-2"><Palette size={20} /> Personalización</h2> <div className="grid grid-cols-2 gap-4 mb-4"> <div><label htmlFor="color-dark" className="block text-sm font-medium text-gray-700 mb-1">Color Puntos</label><input id="color-dark" type="color" value={colorDark} onChange={(e) => setColorDark(e.target.value)} className="w-full h-10 p-1 border rounded-md cursor-pointer" /></div> <div><label htmlFor="color-light" className="block text-sm font-medium text-gray-700 mb-1">Color Fondo</label><input id="color-light" type="color" value={colorLight} onChange={(e) => setColorLight(e.target.value)} className="w-full h-10 p-1 border rounded-md cursor-pointer" /></div> </div> <div> <label htmlFor="logo-upload" className="block text-sm font-medium text-gray-700 mb-1">Logo (opcional)</label> <input id="logo-upload" type="file" accept="image/png, image/jpeg, image/svg+xml" onChange={handleLogoUpload} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-brand-primary/10 file:text-brand-primary hover:file:bg-brand-primary/20 cursor-pointer" /> {logo && <button onClick={() => setLogo(null)} className="text-xs text-red-500 hover:underline mt-2">Quitar logo</button>} </div> </div>
                 <div className="bg-light-bg p-4 rounded-lg border"> <h2 className="font-semibold text-lg mb-4 flex items-center gap-2"><Frame size={20} /> Marcos</h2> <div className="flex flex-wrap gap-2"> <button onClick={() => setFrame('none')} className={`px-3 py-2 text-sm rounded-lg border-2 ${frame === 'none' ? 'border-brand-primary bg-brand-primary/10' : 'border-gray-200'}`}>Sin Marco</button> <button onClick={() => setFrame('scan-me-1')} className={`px-3 py-2 text-sm rounded-lg border-2 ${frame === 'scan-me-1' ? 'border-brand-primary bg-brand-primary/10' : 'border-gray-200'}`}>Scan Me #1</button> <button onClick={() => setFrame('scan-me-2')} className={`px-3 py-2 text-sm rounded-lg border-2 ${frame === 'scan-me-2' ? 'border-brand-primary bg-brand-primary/10' : 'border-gray-200'}`}>Scan Me #2</button> </div> </div>
            </div>
            <div className="bg-light-bg rounded-lg p-6 flex flex-col items-center justify-center border-2 border-dashed">
                ...
                <div className="w-80 h-80 bg-white p-2 rounded-lg shadow-md mb-6 flex items-center justify-center"> {qrCodeDataUrl ? <img src={qrCodeDataUrl} alt="Generated QR Code" style={{maxWidth: '100%', maxHeight: '100%'}} /> : <div className="text-gray-400 text-center">Introduce datos para generar el QR</div>} </div>
                ...
                <canvas ref={finalCanvasRef} width="400" height="400" style={{ display: 'none' }}></canvas>
                 <div className="relative w-full">
                    <button  onClick={() => setDownloadMenuOpen(!isDownloadMenuOpen)} disabled={!qrCodeDataUrl}  className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-brand-primary to-brand-secondary text-white font-bold py-3 px-6 rounded-lg shadow-lg hover:scale-105 transition-transform disabled:opacity-50 disabled:cursor-not-allowed"> <Download size={20} /> Descargar <ChevronDown size={20} className={`transition-transform ${isDownloadMenuOpen ? 'rotate-180' : ''}`} /> </button>
                    {isDownloadMenuOpen && (
                        <div className="absolute bottom-full mb-2 w-full bg-white rounded-lg shadow-xl border overflow-hidden z-10">
                            <button onClick={handlePngDownload} className="w-full text-left px-4 py-3 hover:bg-gray-100">PNG (con marco y logo)</button>
                            <button onClick={handleSvgDownload} className="w-full text-left px-4 py-3 hover:bg-gray-100 border-t">SVG (solo QR, sin marco/logo)</button>
                        </div>
                    )}
                </div>
            </div>
