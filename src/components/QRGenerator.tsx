...
// <-- NUEVO: Añadido 'vcard' a los tipos de QR
...
export function QRGenerator() {
    ...
    // <-- NUEVO: Estado para los datos del vCard -->
    ...
    useEffect(() => {
        ...
        switch (qrType) {
            ...
            // <-- NUEVO: Lógica para construir el string de vCard -->
            case 'vcard':
    ...
    }, [inputValue, emailData, smsData, wifiData, phoneData, vCardData, qrType, colorDark, colorLight, logo, dotStyle, eyeStyle, qrInstance]);
    ...
    const handleTypeChange = (newType: QRType) => {
        setQrType(newType);
        // Reiniciar todos los estados para evitar datos cruzados
    ...
    const renderForm = () => {
        switch (qrType) {
            ...
            // <-- NUEVO: Formulario para vCard -->
            case 'vcard':
                return (
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <input type="text" value={vCardData.firstName} onChange={(e) => setVCardData({...vCardData, firstName: e.target.value})} placeholder="Nombre" className="w-full p-2 border rounded-md"/>
                            <input type="text" value={vCardData.lastName} onChange={(e) => setVCardData({...vCardData, lastName: e.target.value})} placeholder="Apellidos" className="w-full p-2 border rounded-md"/>
                        </div>
                        <input type="tel" value={vCardData.phone} onChange={(e) => setVCardData({...vCardData, phone: e.target.value})} placeholder="Teléfono" className="w-full p-2 border rounded-md"/>
                        <input type="email" value={vCardData.email} onChange={(e) => setVCardData({...vCardData, email: e.target.value})} placeholder="Email" className="w-full p-2 border rounded-md"/>
                        <input type="text" value={vCardData.company} onChange={(e) => setVCardData({...vCardData, company: e.target.value})} placeholder="Empresa" className="w-full p-2 border rounded-md"/>
                        <input type="text" value={vCardData.title} onChange={(e) => setVCardData({...vCardData, title: e.target.value})} placeholder="Cargo" className="w-full p-2 border rounded-md"/>
                        <input type="url" value={vCardData.website} onChange={(e) => setVCardData({...vCardData, website: e.target.value})} placeholder="Sitio Web" className="w-full p-2 border rounded-md"/>
                    </div>
                );
            default: return null;
        }
    ...
    return (
        <div className="w-full h-screen max-h-screen bg-gray-100 flex overflow-hidden">
            ...
            {/* PANEL CENTRAL Y DERECHO (SIN CAMBIOS) */}
            <div className="flex-1 p-8 overflow-y-auto">
                 <div className="max-w-xl mx-auto space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm">
                        ...
                        <div className="grid grid-cols-2 gap-4"> <div><label className="text-sm font-medium text-gray-600">Color Puntos</label><input type="color" value={colorDark} onChange={(e) => setColorDark(e.target.value)} className="w-full h-10 mt-1 p-1 border rounded-md cursor-pointer"/></div> <div><label className="text-sm font-medium text-gray-600">Color Fondo</label><input type="color" value={colorLight} onChange={(e) => setColorLight(e.target.value)} className="w-full h-10 mt-1 p-1 border rounded-md cursor-pointer"/></div> </div>
                        <div className="mt-4 grid grid-cols-2 gap-4"> <div><label className="text-sm font-medium text-gray-600">Estilo Puntos</label><select onChange={(e) => setDotStyle(e.target.value as DotType)} value={dotStyle} className="w-full p-2 mt-1 border rounded-md bg-white">{dotStyleOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.name}</option>)}</select></div> <div><label className="text-sm font-medium text-gray-600">Estilo Ojos</label><select onChange={(e) => setEyeStyle(e.target.value as CornerSquareType)} value={eyeStyle} className="w-full p-2 mt-1 border rounded-md bg-white">{eyeStyleOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.name}</option>)}</select></div> </div>
                    </div>
            ...
            <div className="w-1/3 bg-gray-200 p-8 flex flex-col items-center justify-center">
                 ...
                 <div className="relative w-full max-w-xs mt-8">
                    ...
                    {isDownloadMenuOpen && (
                        <div className="absolute bottom-full mb-2 w-full bg-white rounded-lg shadow-xl border z-10">
